...
function get_partner_names() {
    return array_merge(
            get_mv_partner_names(),
            get_k2_partner_names()
            );
}
...
function get_partners_info() {
    return array_merge(
            get_mv_partners_info(),
            get_k2_partners_info());
}
...
function getv($key, $arr) {
    $keyu = strtoupper($key);
    if(isset($arr[$keyu])) return $arr[$keyu];
    else return '.'.$key; // from nowhere
}
