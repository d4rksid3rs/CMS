...
if($row = mysql_fetch_assoc($result)) {
    ...
    if(!empty($value)) {
        ...
        $from_date = !empty($arr['sms']['from_date']) ? $arr['sms']['from_date'] : '';
        $arr1 = explode(' ', $from_date); $from_date = $arr1[0]; $from_time = $arr1[1];
        $to_date = !empty($arr['sms']['to_date']) ? $arr['sms']['to_date'] : '';
        $arr1 = explode(' ', $to_date); $to_date = $arr1[0]; $to_time = $arr1[1];
        $factor = !empty($arr['sms']['factor']) ? $arr['sms']['factor'] : 1.0;
        // card 1
        $from_date2 = !empty($arr['card']['from_date']) ? $arr['card']['from_date'] : '';
        $arr1 = explode(' ', $from_date2); $from_date2 = $arr1[0]; $from_time2 = $arr1[1];
        $to_date2 = !empty($arr['card']['to_date']) ? $arr['card']['to_date'] : '';
        $arr1 = explode(' ', $to_date2); $to_date2 = $arr1[0]; $to_time2 = $arr1[1];
        $factor2 = !empty($arr['card']['factor']) ? $arr['card']['factor'] : $ratio_presets;
        // card 2
        $from_date3 = !empty($arr['card2']['from_date']) ? $arr['card2']['from_date'] : '';
        $arr1 = explode(' ', $from_date3); $from_date3 = $arr1[0]; $from_time3 = $arr1[1];
        $to_date3 = !empty($arr['card2']['to_date']) ? $arr['card2']['to_date'] : '';
        $arr1 = explode(' ', $to_date3); $to_date3 = $arr1[0]; $to_time3 = $arr1[1];
        $factor3 = !empty($arr['card2']['factor']) ? $arr['card2']['factor'] : $ratio_presets;
    }
}
if($init) {
    $from_date = $to_date = $from_date2 = $to_date2 = $from_date3 = $to_date3 = '';
    $from_time = $from_time2 = $from_time3 = '00:00:00';
    $to_time = $to_time2 = $to_time3 = '23:59:59';
    $factor = 1.0;
                ...
                <div class="box_body">
					<div style="padding-left:10px;">                            
						<form action="" method="post">
								...
								<tbody>
									<?php
										foreach ($msg as $m) {
											...
											echo "<td>";
											echo ($m['os_type'] != "")? $m['os_type']:"Cả 3";echo "</td>";
											if($m['gsm']==0)
												echo "<td>Có</td>";
											else
												echo "<td>Không</td>";
							...
							<input type="button" value="Thêm nhiệm vụ" id='btnThemid' onclick="document.getElementById('divAddEvent').style.display = 'block'; this.style.display = 'none';" />
							<div style="display: none" id="divAddEvent">
								...
								<input type="button" value="Huỷ" onclick="document.getElementById('divAddEvent').style.display = 'none';document.getElementById('btnThemid').style.display = 'block';"/> 
							</div>
